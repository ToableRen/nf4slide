		...
		items : [
			{
				...
				action : function() {
					$("#slide-" + ExId).remove(); $("#tabs-" + ExId).remove()
				}
			},
			{
				label : '向上添加幻灯片',
	...
	$("#" + idBase + idExtend).contextPopup({
		...
		items : [
			...
			},
			{
				...
				action : function() {
					$("#ImageProperty").trigger("click");$("#" + idBase + idExtend).find("img")[0].setAttribute("data-selected", "true")
				}
			}
		]
	});
...
//往id中添加一个代码组件  id可以是类名 亦可以是id,用jquery的选择器的规则
function addCodeToSlide(id) {
  ...
  $("#nf4-code-"+ idExtend).contextPopup({
      //title: '选项',
      items: [
          {label:'删除此代码框',icon:'http://120.24.186.116/Myicons/nf4-view-delete.png', action:function(){$("#nf4-code-"+ idExtend).remove(); } },
          {label:'层次:置顶', icon:'http://120.24.186.116/Myicons/nf4-view-up.png',    action:function() { $("#nf4-code-"+ idExtend).css('z-index',searchMaxZindex()+1);} },
          //null, /* null can be used to add a separator to the menu items */
          {label:'层次:置底',     icon:'http://120.24.186.116/Myicons/nf4-view-down.png',  action:function() { $("#nf4-code-"+ idExtend).css('z-index',searchMinZindex()-1); } },
      ]
  });
		...
		$(thisId).contextPopup({
			...
			items : [
				...
				},
				{
					...
					icon : 'http://120.24.186.116/Myicons/nf4-attr.png',
					action : function() {
						$("#ImageProperty").trigger("click");$(thisId).find("img")[0].setAttribute("data-selected", "true")
					}
				}
			]
		});
